...
- Untied total: `V*D` (token embed) + `max_seq_len*D` (pos embed) + `num_layers * 12*D**2` (blocks) + `2*D` (final LN) + `V*D + V` (head). For `V=8000`, `D=192`, `num_layers=6`, `max_seq_len=256`: `1,536,000 + 49,152 + 2,654,208 + 384 + 1,544,000 = 5,783,744`.
- Tied total: drop one `V*D` term — the head's weight matrix. `5,783,744 - V*D = 5,783,744 - 1,536,000 = 4,247,744`.
- Savings as a fraction of untied total ≈ `1,536,000 / 5,783,744 ≈ 26.6%`.
- The intuition: at this `D`, embeddings are a meaningful fraction of the total. As `D` grows, the `12*D**2`-per-block term dominates and the relative tying savings shrink.

Acceptable variations: small differences in the per-block count (12 vs ~12) are fine if the student is consistent. Accept any answer that correctly identifies the saved term as exactly one `(V, D)` matrix.

Common issues:

- Forgetting that tying still keeps the per-token output bias of size `V` (the savings are `V*D`, not `V*D + V`).
- Counting the embedding savings as `2*V*D` instead of `V*D` (the input lookup is still there — only the *output* matrix disappears).
- Treating positional embeddings as also tied (they aren't — only token embeddings).

## Exercise 09B.09 — Sanity-check tied training

A correct answer should include:

- Both models' initial loss is close to `log(V)`. Exact equality is not expected because the parameter initializations differ.
- Both losses drop after a few SGD steps on the fixed batch.
- The student noticed that gradients flow back into `token_embed.weight` from BOTH the input-lookup path AND the unembedding's matmul when `tie_embeddings=True`.

Common issues:

- Comparing loss values across the two models as if they should match exactly.
- Concluding tied training is broken because the loss curves differ early — the parameter counts differ, so the trajectories do too.
