...
![Four-panel overview of the tool-use loop: tools registered with JSON schemas, rendered into the system prompt, model tool calls parsed and dispatched, and results fed back until the model emits a final answer.](18-tools/Module18-Hero.png)
...
![Four-stage pipeline from model text to tool result: parse the tool-call block, validate arguments against the schema, dispatch through the registry, and feed results or tool_error tags back — errors are conversation, not crashes.](18-tools/Module18-Parse.png)
