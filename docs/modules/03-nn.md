...
Open or resume the working notebook with `.venv/bin/python scripts/open_notebook.py 03`. The notebook contains runnable experiments and detailed prompts; implementation work lives in `g2c/nn/`.
...
5. **Remove the nonlinearity.** Show why stacked linear layers collapse to one linear map.
