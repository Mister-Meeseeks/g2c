...
Each node in the neural network adds `fan_in` number random activation inputs. Therefore to keep activations scaled across layers, weights need to be normalized by `fan_in`. The course's neural network module initializes:

```
W ~ Uniform(-1/√fan_in, +1/√fan_in)
```

The named schemes in the literature are this same argument with sharpened constants: **Xavier/Glorot** balances fan-in against fan-out for symmetric activations like tanh; **Kaiming/He** adds a factor of √2 because ReLU zeroes half its inputs.
