...
* *Review* [01-autodiff](01-autodiff.md) and [02-tensors](02-tensors.md) for the building blocks and [00-prerequisite-review](00-prerequisite-review.md) for the basics.
* *Review* [PyTorch Primer](../primers/pytorch.md) if any of the PyTorch code feels unfamiliar or confusing, especially tensor basics.
* *Review* the [Probability & cross-entropy primer](../primers/probability-cross-entropy.md) for why the loss is −log p and where the (p − y) gradient comes from.
...
Initialization determines the starting value of the weights in the neural network, before any training step runs. Initialization always uses random (zero centered) values, so the primary design decision becomes how to **scale** those random numbers. That scale is represented in the standard devision `σ`.

When selecting `σ` in a deep neural network, the primary goal is to control the **amplification factor**. Because activations is repeatedly amplified over many stacked layers, mis-calibration can create fatal stability issues in training. The right scale keeps the *learning signal* alive, not just the forward pass. Too large, and the gradient explodes. Too small, and the gradient vanishes. Either way useful signal fails to propagating to earlier layers. 

Each node in the neural network combines `fan_in` number of random activations as inputs. Therefore the keep outputs scaled
The course's neural network module initializes:

```
W ~ Uniform(-1/√fan_in, +1/√fan_in)
```

The named schemes in the literature are this same argument with sharpened constants: **Xavier/Glorot** balances fan-in against fan-out for symmetric activations like tanh; **Kaiming/He** adds a factor of √2 because ReLU zeroes half its inputs and the surviving half must carry the variance. Module 03B's initialization-sweep exercise has you discover Kaiming's constant empirically, off a plot.


The goal is for the initialized weights at each layer to output activations that are roughly the same magnitude as the inputs to the layer. 

Initialization is about where we start at step 0. Before the first training step, every weight starts as random number. The design decision is about how to **scale** those random numbers. 

Start by considering the output at each linear layer. Each output reduces down to a sum of `fan_in` random terms:

``` 
y_j = Σ_i x_i · W_ij
```

When initializing the weights we choose a standard deviation `σ`.  The variance of the output is a function of the variance of the weights and inputs:

```
var(y_j) = fan_in · σ² · var(x) 
```

The output activation at one layer becomes the input at the next. For each layer there's a per-layer amplification factor (`L`) which stretches or shrinks the random activations from input to output:

```
L ≈ var(y)/var(x)
L ≈ [fan_in · σ² · var(x)] / var(x)
L ≈ fan_in · σ²
```

Now consider stacking layers. The amplification from model inputs to model outputs (`M`) is the exponent of the per-layer amplification by the number of layers. The deep neural networks we use, by definition, have a large number of layers. That translates into a large exponent:
...
If `L` deviates from unity, then `M` will drastically deviate from unity when applied across many stacked layers. To avoid the issues that arise from that, the random initialization of weights must scale based on the width of the network: 
...
Gradients obey the same arithmetic. The right scale keeps the *learning signal* alive, not just the forward pass. Too large, and the gradient explodes. Too small, and the gradient vanishes. Either way useful signal fails to propagating to earlier layers. 


Two failure signatures to be aware of, both readable from a loss curve:

- **Loss starts at uniform baseline and crawls.** Weights too small. The forward signal, and with it, the gradient, has decayed toward zero. Each extra layer multiplies the escape time, until a deep net is effectively frozen at the baseline.
- **Loss starts far *above* the baseline.** Weights too large. The softmax is saturated and the model is confidently wrong. A fresh model should know nothing, and knowing nothing has an exact numeric value. The [probability primer](../primers/probability-cross-entropy.md) explains why.
...
- **Initialization**. Randomize weights to scale, but use a scale that will keeps output magnitudes stable across stacked layers. 
...
Tests are in `tests/test_nn.py`. Initial state: 13 passed, 27 failed.
Construction and bookkeeping tests pass from the start; forward passes, losses,
optimizer steps, and the training helpers turn green as you implement the
TODOs.
...
- **Trying to use non-differentiable metrics for training.** MSE and cross-entropy are differentiable losses: they tell autograd how to change the parameters. Many accuracy metrics are useful but non-differentiable and therefore can't be used with autograd. Average predicted-class accuracy, for example.
...
- **Glorot, Bengio, "Understanding the difficulty of training deep feedforward neural networks" (2010).** Where the fan-based initialization argument in "Where weights start" comes from.
- **He, Zhang, Ren, Sun, "Delving Deep into Rectifiers" (2015).** The ReLU-corrected variant — the √6 constant Module 03B's initialization sweep rediscovers.
