...
- MNIST with a small MLP trains in ~1–2 minutes per epoch on CPU. The course `Module` base now has `.to(device)`, so larger notebook experiments can move the model with `model.to("mps")` or let later helpers use `device="auto"`.
