...
Before the first gradient step, every weight is a random number, and the only design decision in that randomness is its **scale**. The scale is load-bearing.

Look at one output of a linear layer: `y_j = Σ_i x_i · W_ij` — a sum of `fan_in` random terms. If the inputs have roughly unit variance and the weights have standard deviation `σ`, the output's variance is about `fan_in · σ²`. Now stack layers: whatever per-layer gain that works out to, `L` layers raise it to the `L`-th power. Exponential growth, exponential decay — or, at exactly one scale, neither. Choosing `σ ≈ 1/√fan_in` cancels the `fan_in` factor: unit-scale signal in, unit-scale signal out, at any depth. And gradients obey the same shape arithmetic — they flow back through the same matrices, transposed (see the [matrix calculus primer](../primers/matrix-calculus.md)) — so the right scale keeps the *learning signal* alive, not just the forward pass.

This is why the course's `Linear` initializes `W ~ Uniform(-1/√fan_in, +1/√fan_in)` — open `g2c/nn/modules.py` and find the line; you'll train against it in every module from here on. The named schemes in the literature are this same argument with sharpened constants: **Xavier/Glorot** balances fan-in against fan-out for symmetric activations like tanh; **Kaiming/He** adds a factor of √2 because ReLU zeroes half its inputs and the surviving half must carry the variance. Module 03B's initialization-sweep exercise has you discover Kaiming's constant empirically, off a plot.

Two failure signatures, both readable from a loss curve before you suspect your architecture:

- **Loss starts at the uniform baseline `−log(1/C)` and crawls.** Weights too small: the forward signal — and with it, the gradient — has decayed toward zero. A shallow net eventually escapes; each extra layer multiplies the escape time, until a deep net is effectively frozen at the baseline.
...
Initialization only controls step 0; the course returns twice to what keeps the property alive afterwards. Module 07's `√D` attention scaling silently assumes unit-variance vectors — an assumption this section is what pays for. And Module 09's LayerNorm re-imposes unit scale at every block input, which is a large part of why genuinely deep stacks train at all.
