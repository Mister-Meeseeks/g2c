...
*Greedy and pure-random are the two failure modes; the rest of the module is about the middle column. The warpers shape the model's native distribution into something that's neither argmax-locked nor long-tail-derailed, so the multinomial draw lands somewhere plausible. Every example output you'll generate in the exercises sits on the spectrum drawn under the trains here.*
...
*The reason top-p tends to read as "smarter" than top-k in practice: it spends its budget where the budget actually matters. Confident step → narrow nucleus, no distractors slip in. Uncertain step → wider nucleus, no plausible candidates get cut off. A fixed top-k can't do both. In the exercises, this is what the side-by-side comparison in Exercise 2 is meant to show — same prompt, same temperature, different cutoff rule, visibly different output character.*
