"""String matchers for the generation harness.

A matcher is `Callable[[str, list[str]], bool]` — given the model's
prediction and a list of acceptable references, return True iff
prediction matches any reference.

Four matchers, in order of strictness:

  * `exact_match`      — character-for-character equality.
  * `normalized_match` — case/punctuation/whitespace normalized.
  * `contains_match`   — any reference appears as a substring.
  * `numeric_match`    — extract a number from prediction and
                         compare to numbers extracted from references.

The matchers are independent; the test file exercises each. The
harness uses whichever you pass it. Conventional pairings:

  * Factual Q&A:           normalized_match (the model gets a few
                            tries on capitalization).
  * Multiple-choice text:  exact_match (you control both sides).
  * Arithmetic:            numeric_match (the model often pads "= ").
  * Keyword instruction:   contains_match (the model emits prose).

Why these four and not, say, BLEU or ROUGE: at toy scale the model
emits very short responses; reference-style metrics over n-grams
give noisy numbers that don't track behavior. Strict matching is
both more interpretable and a tighter bound on capability.
"""
