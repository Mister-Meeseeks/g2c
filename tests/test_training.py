...
Suggested order to implement & turn green:
  ...
  2. cosine_with_warmup           → test_cosine_with_warmup_*
                                    + test_trainer_lr_*
  3. clip_grad_norm_              → test_clip_grad_norm_*
  4. Trainer.train_step           → test_trainer_train_step_*,
...
def test_cosine_with_warmup_during_warmup_is_linear():
    ...
    assert lr_step9 == pytest.approx(1e-3)  # last warmup step → exactly max_lr
...
def test_cosine_with_warmup_halfway_through_decay():
    """Halfway through the cosine phase, cos(π/2) = 0 so the coefficient
...
def test_cosine_with_warmup_at_max_steps():
    """At step == max_steps, cos(π) = -1, coeff = 0, so lr = min_lr."""
    lr = cosine_with_warmup(
...
def test_cosine_with_warmup_after_max_steps_held_at_min_lr():
    """Past max_steps, the function clamps at min_lr — does not crash,
    does not grow.
    """
...
def test_clip_grad_norm_returns_pre_clip_norm():
    """The returned float is the PRE-clip total norm — useful for
    logging how often clipping fires.
    """
    p = _make_param_with_grad([3.0, 4.0])  # norm = 5.0
...
def test_clip_grad_norm_global_across_params():
    """Norm pools across ALL params — not per-param.

    Two params, each with a grad of norm 3 and 4 respectively, give a
    GLOBAL norm of sqrt(3² + 4²) = 5. After clipping to max_norm=1, the
    new global norm should be 1, with each per-param grad shrunk by
    1/5.
    """
...
def test_clip_grad_norm_skips_none_grads():
    """Params with `grad=None` are skipped (they didn't participate)."""
    ...
    assert norm == pytest.approx(5.0, abs=1e-3)
    # The None-grad param should still have None grad — clipping
    # didn't accidentally allocate one.
    assert p_none.grad is None


def test_clip_grad_norm_empty_returns_zero():
    """No params (or all-None grads) → total norm is 0; no error."""
